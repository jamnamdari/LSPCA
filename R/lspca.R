...
Truncate_fun <- function(U, s){
  if(is.vector(U)){
    p <- length(U)
    #U_trunc <- matrix(0, nrow = length(U), ncol = 1)
...
LSPCA0 <- function(n,p, f_xx, Pi, d, lambda, lr = 0.02, maxiter = 60,
  ...
  if(d == 1){
    return(LSDPC_fps_SOAP_PD_fun(n,p, f_xx, Pi,d=1, lambda, lr = 0.02, maxiter = 60,
                                 control = list(fan_maxinc = 10, verbose = 0), eta, s, n_iter, nu))
  }else if(d==2){
    return(LSDPC_fps_SOAP_PD2_fun(n,p, f_xx, Pi,d=2, lambda, lr = 0.02, maxiter = 60,
                                  control = list(fan_maxinc = 10, verbose = 0), eta, s, n_iter, nu))
  }else{
    return(LSDPC_fps_SOAP_PDd_fun(n,p, f_xx, Pi,ds=d, lambda, lr = 0.02, maxiter = 60,
                                  control = list(fan_maxinc = 10, verbose = 0), eta, s, n_iter, nu))
  }
}
  ...
  for(ell in 1:len_freq){
    ...
    if(ell < len_freq){
      if(ell < 10){
        ...
        tryCatch({ out_1 <-  fps::fps(S[,,ell], ndim = 2)
        i_1 <- which.min(abs(out_1$lambda - lambda))
        fxx_grad_ell <- as.numeric(out_1$projection[[i_1]])
...
sparse_par_selector1 <- function(X, eta, nu=0){
  ...
  for(k in 1:20){
    ...
    LSDPCA_ADMM_SOAP_Ex1 <- LSDPC_fps_SOAP_PD_fun(n,p, f_xx1, Pi <- matrix(0, nrow = 2*p, ncol=2*p), lambda = 0.5 * sqrt(log(p) / n), lr = 0.02, maxiter = 60,
  ...
  for(ell in 1:len_freq){
    ...
    if(ell < len_freq){
      if(ell < 10){
        ...
        tryCatch({ out_1 <-  fps::fps(S[,,ell], ndim = 4)
        i_1 <- which.min(abs(out_1$lambda - lambda))
        fxx_grad_ell <- as.numeric(out_1$projection[[i_1]])
        Re_H <- fxx_grad_ell$projection[1:p,1:p]
        Im_H <- fxx_grad_ell$projection[1:p,(p+1):(2*p)]
...
sparse_par_selector2 <- function(X, eta, nu=0){
  ...
  for(k in 1:20){
    ...
    LSDPCA_ADMM_SOAP_Ex1 <- LSDPC_fps_SOAP_PD2_fun(n,p, f_xx1, Pi <- matrix(0, nrow = 2*p, ncol=2*p), lambda = 0.5 * sqrt(log(p) / n), lr = 0.02, maxiter = 60,
                                                   control = list(fan_maxinc = 10, verbose = 0), eta=eta, s=k, n_iter = 20, nu=nu)
  ...
  for(ell in 1:len_freq){
    ...
    if(ell < len_freq){
      if(ell < 10){
        ...
        tryCatch({ out_1 <-  fps::fps(S[,,ell], ndim = 2*ds)
        i_1 <- which.min(abs(out_1$lambda - lambda))
        fxx_grad_ell <- as.numeric(out_1$projection[[i_1]])
        Re_H <- fxx_grad_ell$projection[1:p,1:p]
        Im_H <- fxx_grad_ell$projection[1:p,(p+1):(2*p)]
...
sparse_par_selector_d <- function(X, eta, nu=0, ds){
  ...
  for(k in 1:20){
    ...
    LSDPCA_ADMM_SOAP_Ex1 <- LSDPC_fps_SOAP_PDd_fun(n,p, f_xx1, Pi <- matrix(0, nrow = 2*p, ncol=2*p), ds, lambda = 0.5 * sqrt(log(p) / n), lr = 0.02, maxiter = 60,
                                                   control = list(fan_maxinc = 10, verbose = 0), eta=eta, s=k, n_iter = 20, nu=nu)
